...
import logging
import utils_validations.validations as validations
...
def validate_orders_payments(orders_payments_df: DataFrame) -> DataFrame:
    ...
    valid_payments_records = {}

    # Validations
    valid_payments_records['order_id'], invalid_payments_records['order_id'] = validations.validate_alphanumeric_id_32(
        orders_payments_df, 'order_id'
    )
    
    valid_payments_records['payment_type'], invalid_payments_records['payment_type'] = validations.validate_payment_method_format(
        orders_payments_df, 'payment_type'
    )
    
    valid_payments_records['payment_sequential'], invalid_payments_records['payment_sequential'] = validations.validate_integer_format(
        orders_payments_df, 'payment_sequential'
    )
    
    valid_payments_records['payment_installments'], invalid_payments_records['payment_installments'] = validations.validate_integer_format(
        orders_payments_df, 'payment_installments'
    )
    
    valid_payments_records['payment_value'], invalid_payments_records['payment_value'] = validations.validate_monetary_format(
