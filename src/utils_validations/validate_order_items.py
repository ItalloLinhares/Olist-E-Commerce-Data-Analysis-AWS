...
def validate_order_items(order_items_df: DataFrame) -> DataFrame:
    ...
    for col in ids_to_check:
        valid_items_records[col], invalid_items_records[col] = validations.validate_alphanumeric_id_32(
            order_items_df, col
        )

    # Validate Date
    valid_items_records['shipping_limit_date'], invalid_items_records['shipping_limit_date'] = validations.validate_datetime_format(
        order_items_df, 'shipping_limit_date'
    )
    
    # Validate Monetary Values
    valid_items_records['price'],invalid_items_records['price'] = validations.validate_monetary_format(
        order_items_df, 'price'
    )

    valid_items_records['freight_value'],invalid_items_records['freight_value'] = validations.validate_monetary_format(
