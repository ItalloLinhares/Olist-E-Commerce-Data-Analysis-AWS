...
def validate_orders(orders_df: DataFrame) -> DataFrame:
    ...
    valid_orders_records = {}
    # --- VALIDATION STEPS ---
    
    # Validate order_id format
    valid_orders_records['order_id'], invalid_orders_records['order_id'] = validations.validate_alphanumeric_id_32(
        orders_df, 'order_id'
    )
    
    # Validate customer_id format
    valid_orders_records['customer_id'], invalid_orders_records['customer_id'] = validations.validate_alphanumeric_id_32(
        orders_df, 'customer_id'
    )
    
    # Validate order_status format
    valid_orders_records['order_status'], invalid_orders_records['order_status'] = validations.validate_order_status_format(
    ...
    for col_name in datetime_columns:
        valid_orders_records[col_name], invalid_orders_records[col_name] = validations.validate_datetime_format(
            orders_df, col_name
        )
    ...
    #Adding partion Column
    cleaned_df = cleaned_df.withColumn(
    "order_purchase_month_and_year", 
    F.date_format(F.to_timestamp("order_purchase_timestamp", "dd-MM-yyyy HH:mm:ss"), "yyyy-MM")
)
