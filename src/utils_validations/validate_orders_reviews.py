...
def validate_orders_reviews(orders_reviews_df: DataFrame) -> DataFrame:
    ...
    valid_reviews_records = {}

    # Validate IDs
    valid_reviews_records['review_id'], invalid_reviews_records['review_id'] = validations.validate_alphanumeric_id_32(
        orders_reviews_df, 'review_id'
    )

    valid_reviews_records['order_id'], invalid_reviews_records['order_id'] = validations.validate_alphanumeric_id_32(
        orders_reviews_df, 'order_id'
    )

    # Validate Score
    valid_reviews_records['review_score'], invalid_reviews_records['review_score'] = validations.validate_score_format(
        orders_reviews_df, 'review_score'
    )
    
    # Validate Dates
    valid_reviews_records['review_creation_date'], invalid_reviews_records['review_creation_date'] = validations.validate_datetime_format(
        orders_reviews_df, 'review_creation_date'
    )

    valid_reviews_records['review_answer_timestamp'], invalid_reviews_records['review_answer_timestamp'] = validations.validate_datetime_format(
